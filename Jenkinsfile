pipeline {
    ...
    stages {
        stage('Get AWS Account ID') {
            steps {
                script {
                    ...
                    env.FULL_IMAGE_URL = "${env.ECR_REGISTRY}/${ECR_REPO}:latest"
                }
            }
        }
        ...
        stage('Push to ECR') {
            steps {
                script {
                    ...
                    sh "docker tag ${ECR_REPO}:latest ${FULL_IMAGE_URL}"
        ...
        stage('Deploy to Staging') {
            steps {
                script {
                    ...
                    echo "Injecting Docker Image URL into Staging YAML..."
        ...
        stage('Deploy to Production') {
            steps {
                script {
                    echo "Injecting Docker Image URL into Production YAML..."
